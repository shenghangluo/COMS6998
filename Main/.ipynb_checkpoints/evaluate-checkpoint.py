...
def load_model(checkpoint_path: str, config: Config, tokenizer, use_lora: bool = False):
    ...
    if use_lora:
        ...
        if backbone_path.exists() and mlp_head_path.exists():
            model.backbone.load_state_dict(torch.load(backbone_path, map_location='cpu'))
            model.mlp_head.load_state_dict(torch.load(mlp_head_path, map_location='cpu'))
        else:
...
def evaluate_model(model, dataloader, save_predictions: bool = False):
    ...
    for batch_idx, batch in enumerate(tqdm(dataloader, desc="Evaluating")):
        ...
        for i in range(predictions.size(0)):
            ...
            if valid_mask.any():
                pred_valid = predictions[i][valid_mask].cpu().numpy()
                label_valid = labels[i][valid_mask].cpu().numpy()
